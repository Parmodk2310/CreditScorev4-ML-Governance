...
def test_public_entrypoint_uses_https_and_hardened_alb() -> None:
    ecs = (ROOT / "infra/terraform/ecs.tf").read_text()
    outputs = (ROOT / "infra/terraform/outputs.tf").read_text()

    assert 'protocol          = "HTTPS"' in ecs
    assert "certificate_arn = var.acm_certificate_arn" in ecs
    assert "drop_invalid_header_fields = true" in ecs
    assert 'value = "https://${aws_lb.app.dns_name}"' in outputs


def test_public_subnets_do_not_auto_assign_public_ips() -> None:
    networking = (ROOT / "infra/terraform/networking.tf").read_text()

    assert "map_public_ip_on_launch = false" in networking
