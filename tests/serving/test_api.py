...
def test_online_contract_rejects_age_outside_governed_range(monkeypatch, tmp_path: Path) -> None:
    payload = _payload()
    payload["age"] = 90
...
def test_online_contract_rejects_unknown_region(monkeypatch, tmp_path: Path) -> None:
    payload = _payload()
